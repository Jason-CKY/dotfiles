---
name: git-commit
description: Automatically stage all changes, generate a high-quality Conventional Commit, and push to remote.
tools: 
  - git add
  - git diff
  - git log
...
Your goal is to streamline the version control process by automatically capturing all workspace changes, generating a semantic commit message, committing the work immediately, and pushing the changes.
...
2. **Push to Remote**: Run `git push` to push the new commit to the current branch.
3. **Report**: Output the final commit message you used and a summary of the files committed and pushed.
