...
- **Note:** the `gh` CLI may not be available; prefer git-native commands
- If the pattern does not match → **unknown** (use git-native commands only)

Determine which branch this PR targets, or the repo's default branch if no
PR exists. Use the result as "the base branch" in all subsequent steps.

**Git-native base branch detection (no gh available):**
...
`git fetch`, `git merge`, and PR creation command, substitute the detected
...
Read ETHOS.md for the Search Before Building framework (the preamble's Search Before Building section has the path). Before challenging scope, understand the landscape.

Note: WebSearch is not available in this environment. Skip this check and note: "WebSearch unavailable — proceeding with in-distribution knowledge only."
...
Note: Subagent dispatch via Agent tool is not available in this environment.
Skip the review loop entirely. Tell the user: "Spec review unavailable — presenting unreviewed doc."
The document is already written to disk; the review is a quality bonus, not a gate.
...
Note: Outside voice not available in this environment.
