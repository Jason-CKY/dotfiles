...
### 0D. Focus Areas
ask_user: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll review all 7 dimensions. Want me to focus on specific areas instead of all 7?"

**STOP.** Do NOT proceed until user responds.

## Step 0.5: Visual Mockups

If the plan involves any UI — screens, pages, components, visual changes — and a design
tool is available, generate mockups to inform the review. This makes design gaps concrete
rather than abstract.

The ONLY time you skip mockups is when:
- The design tool is not available
- The plan has zero UI scope (pure backend/API/infrastructure)
- The user explicitly says "skip mockups" or "text only"

If mockups are generated, they serve as visual reference throughout the review passes.

## Design Outside Voices (not available)

Note: Codex and subagent dispatch are not available in this environment. Skip this step and continue with the primary review.
...
If a dimension rates below 7/10, describe what a 10/10 would look like for that dimension.
Use concrete examples: specific UI patterns, layout approaches, or interaction designs that
would achieve a 10 rating. Make the gap between "what the plan describes" and "what it should
look like" visceral, not abstract.
...
If visual mockups were generated in Step 0.5, evaluate them against the AI slop blacklist above. Read each mockup image using the Read tool. Does the mockup fall into generic patterns (3-column grid, centered hero, stock-photo feel)? If so, flag it.
**STOP.** ask_user once per issue. Do NOT batch. Recommend + WHY.
...
If yes, use the design tool to regenerate with updated direction. Save to the same output directory.
...
* **NEVER use ask_user to ask which variant the user prefers.** If using a comparison board tool, use it to collect preferences. Use ask_user ONLY to notify the user and wait for them to finish — not to present variants inline and ask "which do you prefer?"
...
|----------------|-------------|-----------|-------|
| [screen name]  | ./.design-reviews/[folder]/[filename].png | [brief description] | [constraints from review] |
```

Include the full path to each approved mockup (the variant the user chose), a one-line description of the direction, and any constraints. The implementer reads this to know exactly which visual to build from. If no mockups were generated, omit this section.

## Review Log

After producing the Completion Summary above, persist the review result to the plan file.
...
After completing the review, display the dashboard using available review tools.

**Source attribution:** If the most recent entry for a skill has a \`"via"\` field, append it to the status label in parentheses. Examples: `plan-eng-review` with `via:"autoplan"` shows as "CLEAR (PLAN via /autoplan)". Entries without a `via` field show as "CLEAR (PLAN)" or "CLEAR (DIFF)" as before.
...
- **Outside Voice (optional):** Not available in this environment.

**Verdict logic:**
- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review\` or \`plan-eng-review\` with status "clean"
...
this session, consider logging it for future sessions using the `/update-claude-learnings` skill.
